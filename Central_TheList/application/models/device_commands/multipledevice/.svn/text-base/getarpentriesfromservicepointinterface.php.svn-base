...
class thelist_multipledevice_command_getarpentriesfromservicepointinterface implements Thelist_Commander_pattern_interface_idevicecommand
{
	...
	{
		//object model only as this depends 
			...
			foreach($border_routers as $border_router){
				...
				if ($edge_switches != false) {
					...
					foreach($edge_switches as $edge_switch){
						...
						$credentials = $edge_switch['equipment']->get_apis();
						$credential = current$credentials);
						$device = new Thelist_Model_device($edge_switch['equipment']->get_eq_fqdn(), $credential);
						
						$clearmacaddresses = new Thelist_Cisco_command_removeinterfacecamentries($device, $edge_switch['inbound_interface']);
						$clearmacaddresses->execute();
