...
//exception codes 15800-15899

class thelist_cisco_command_getequipment implements Thelist_Commander_pattern_interface_idevicecommand 
{
	private $_device;
	...
	{
		$get_serial			= new Thelist_Cisco_command_getserialnumber($this->_device);
		...
		if ($this->_equipment != null) {
			...
			if ($this->_equipment->get_interfaces() != null) {
				foreach($this->_equipment->get_interfaces() as $interface) {
					$get_mac = new Thelist_Cisco_command_getinterfacemacaddress($this->_device, $interface);
