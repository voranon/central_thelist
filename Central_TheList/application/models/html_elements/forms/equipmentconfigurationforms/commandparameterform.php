...
class thelist_equipmentconfigurationform_commandparameterform extends Zend_Form{

	public function __construct($options=null)
	{
		parent::__construct($options);

	$device_command_parameter_name	 = new Zend_Form_Element_Textarea('device_command_parameter_name',
								array(
									'label'      	=> 'Parameter Name:',
									'decorators' 	=> array(new commandparameterdecorator($options))	
	)
	);
	$device_command_parameter_name->setRequired(true);
...
class commandparameterdecorator extends Zend_Form_Decorator_Abstract
{
	private $database;
	...
	public function __construct($options=null){
		...
		if ($this->_function_type == 'edit') {
			...
			$sql =	"SELECT dcp.*, CONCAT(dcpt.table_name, '.', dcpc.column_name) AS parameter FROM device_command_parameters dcp
					...
					WHERE device_command_parameter_id='".$this->_variable."'
					";
	...
	public function render($content)
	{
		...
		if ($name == 'device_command_parameter_name'){
				
			if ($this->_function_type == 'edit') {
				
				$format =	"<fieldset><legend>Edit Command Parameter</legend><table style='width:500px'><tr>
							<td>%s</td><td><input style='width: 300px' name='device_command_parameter_name' id='device_command_parameter_name' type='text' class='text' value='".$this->_row['device_command_parameter_name']."'></input></td>
							</tr>";

			} else {
				
				$format =	"<fieldset><legend>Add Command Parameter</legend><table style='width:500px'><tr>
							<td>%s</td><td><input style='width: 300px' name='device_command_parameter_name' id='device_command_parameter_name' type='text' class='text' value=''></input></td>
							</tr>";
			}
		...
		if ($this->_function_type == 'add') {
			...
			if($name == 'edit'){
				$format="			<tr><td colspan='3' align='center'>
											...
											<input name='device_command_parameter_id' type='hidden' value='".$this->_variable."'></input>
