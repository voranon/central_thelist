...
class thelist_contactform_editcontact extends Zend_Form{

	private $database;


	public function __construct($options = null,$input=null,$contact_id=null)
	{
		...
		if($mode=='project'){
			$sql="SELECT item_id
				  FROM project_contact_mapping pcm
				  LEFT OUTER JOIN items i ON pcm.contact_title=i.item_id
				  WHERE  pcm.project_id=".$mode_id." 
				  AND pcm.contact_id=".$contact_id;
		
			$contact_title = Zend_Registry::get('database')->get_thelist_adapter()->fetchRow($sql);
			$contact_title['item_id'];
			
		}else if($mode=='building'){
			$sql="SELECT item_id
				  FROM building_contact_mapping bcm
				  LEFT OUTER JOIN items i ON bcm.contact_title=i.item_id
				  WHERE  bcm.building_id=".$mode_id." 
				  AND bcm.contact_id=".$contact_id;
		
			$contact_title = Zend_Registry::get('database')->get_thelist_adapter()->fetchRow($sql);
			$contact_title['item_id'];
		}
	
		$contact = Zend_Registry::get('database')->get_contacts()->fetchRow("contact_id=".$contact_id);
		...
		$save_contact = new Zend_Form_Element_Submit('save_contact',array(
																'label'      =>'',
																'value'		 =>'save',
		...
		$delete_contact = new Zend_Form_Element_Submit('delete_contact',array(
																'label'      =>'',
																'value'		 =>'delete',
					   						 					'decorators' => array(new contactsubmit_decorator(),
		...
		$this->addElements(array($title,$firstname,$lastname,$streetnumber,$streetname,$streettype,$city,$state,$zip,$cellphone,$homephone,$officephone,$fax,$email,$save_contact,$delete_contact));
